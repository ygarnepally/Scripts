...
f = open(file_name, 'w+')
f.write("ClusterName,Service,PolicyName,PolicyType,AlarmName,MetricName,Threshold,EvaluationPeriod,Period,Dimensions")
f.write('\n')
        ...
        for service in cluster_services['serviceArns']:
            ...
            for policy in desc_aasg['ScalingPolicies']:
                ...
                for alarm in policy['Alarms']:
                    ...
                    dimensions = ''
                    for dimension in desc_alarm['Dimensions']:
                        dim_str = dimension['Name'] + '=' + dimension['Value']
                        dimensions = dimensions + dim_str + ','



               
                    print(clustername + '\t' + servicename + '\t' + policy_name + '\t' + policy_type + '\t' + alarmname + '\t' + metricname + '\t' + str(threshold) + '\t' + str(evelp) + '\t' + str(period) + '\t' + str(dimensions))
                    #input('proceed..?')
                    f.write(clustername + ',' + servicename + ',' + policy_name + ',' + policy_type + ',' + alarmname + ',' + metricname + ',' + str(threshold) + ',' + str(evelp) + ',' + str(period) + ',' + str(dimensions) + '\n')
